...
-- Table names: stk_adj_sessions, stk_adj_lines, stk_adj_audit
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM stk_adj_sessions WHERE adj_id = @adj_id AND status = 'Draft')
    BEGIN
        RAISERROR('Only Draft sessions can be edited.', 16, 1);
        RETURN;
    END;

    IF EXISTS (SELECT 1 FROM stk_adj_lines WHERE adj_id = @adj_id AND product_id = @product_id)
    BEGIN
        UPDATE stk_adj_lines SET
...
BEGIN
    ...
    DELETE FROM stk_adj_lines WHERE adj_id = @adj_id AND product_id = @product_id;
END;
GO
...
BEGIN
    ...
    FROM stk_adj_sessions WITH (UPDLOCK)
    WHERE adj_id = @adj_id;
    ...
    END;
    IF NOT EXISTS (SELECT 1 FROM stk_adj_lines WHERE adj_id = @adj_id)
    BEGIN
    ...
    JOIN stk_adj_lines l ON l.product_id = ps.product_id
    ...
    FROM mst_products p
    JOIN stk_adj_lines l ON l.product_id = p.id
    WHERE l.adj_id = @adj_id
    ...
    JOIN stk_adj_lines l ON l.product_id = loc.product_id
    JOIN stk_adj_sessions s ON s.adj_id = l.adj_id
    WHERE l.adj_id = @adj_id
    ...
    UPDATE stk_adj_sessions SET
        ...
        posted_at           = GETDATE(),
        modified_at         = GETDATE(),
        total_lines         = (SELECT COUNT(*)   FROM stk_adj_lines WHERE adj_id = @adj_id),
        qty_increases       = (SELECT COUNT(*)   FROM stk_adj_lines WHERE adj_id = @adj_id AND qty_difference  > 0),
        qty_decreases       = (SELECT COUNT(*)   FROM stk_adj_lines WHERE adj_id = @adj_id AND qty_difference  < 0),
        price_changes       = (SELECT COUNT(*)   FROM stk_adj_lines WHERE adj_id = @adj_id AND price_difference <> 0),
        location_changes    = (SELECT COUNT(*)   FROM stk_adj_lines WHERE adj_id = @adj_id
...
-- ============================================================
-- sp_StockVarianceReport  (updated to use stk_adj_audit + mst_products)
...
-- ============================================================
-- sp_PriceChangeReport  (updated to use stk_adj_audit + mst_products)
