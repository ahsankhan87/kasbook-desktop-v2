-- ============================================================================
-- File: sp_GetNonMovingItems.sql
-- Purpose: Retrieve non-moving/stagnant items (dormant inventory with no recent sales)
-- Database Tables:
...
CREATE PROCEDURE [dbo].[sp_GetNonMovingItems]
	@BranchId INT,
...
BEGIN
	...
	SELECT
		...
		QtyMovedLastPeriod,
		CASE
			WHEN TotalTransactions = 0 THEN 'Never Sold'
			WHEN DaysSinceLastSale >= 180 THEN 'Dead Stock'
			ELSE 'Slow Moving'
		END AS StockStatus,
		CASE
			WHEN QtyOnHand >= 100 THEN 'High'
			WHEN QtyOnHand >= 10 THEN 'Medium'
			ELSE 'Low'
...
-- ============================================================================
-- File: sp_GetNonMovingItemsSummary.sql
-- Purpose: Retrieve summary statistics for non-moving items
-- ============================================================================

CREATE PROCEDURE [dbo].[sp_GetNonMovingItemsSummary]
	@BranchId INT,
	@DaysThreshold INT = 90
AS
BEGIN
	...
	SELECT
		COUNT(DISTINCT id) AS TotalNonMovingItems,
		...
		SUM(CASE WHEN TotalTransactions = 0 THEN 1 ELSE 0 END) AS NeverSoldCount,
		SUM(CASE WHEN DaysSinceLastSale >= 180 THEN 1 ELSE 0 END) AS DeadStockCount,
