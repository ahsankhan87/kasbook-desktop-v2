...
-- Run order: 1) pos_stk_adj_sessions  2) stk_adj_lines
--            3) stk_adj_audit     4) pos_product_locations
--            5) sp_GenerateAdjNo
-- ============================================================
...
-- ============================================================
-- 1. pos_stk_adj_sessions — Adjustment session header
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'pos_stk_adj_sessions')
BEGIN
    CREATE TABLE pos_stk_adj_sessions (
        adj_id              INT             NOT NULL IDENTITY(1,1),
        adj_no              VARCHAR(20)     NOT NULL,
        adj_date            DATE            NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_adj_date     DEFAULT CAST(GETDATE() AS DATE),
        adj_type            VARCHAR(30)     NOT NULL,
        warehouse_id        INT             NOT NULL,
        status              VARCHAR(10)     NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_status        DEFAULT 'Draft',

        -- Rolling counters (updated by sp_PostStockAdjustment)
        total_lines         INT             NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_total_lines   DEFAULT 0,
        qty_increases       INT             NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_qty_inc       DEFAULT 0,
        qty_decreases       INT             NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_qty_dec       DEFAULT 0,
        price_changes       INT             NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_price_chg     DEFAULT 0,
        location_changes    INT             NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_loc_chg       DEFAULT 0,

        notes               NVARCHAR(500)   NULL,

        -- Lifecycle timestamps & actors
        created_by          INT             NOT NULL,
        created_at          DATETIME        NOT NULL CONSTRAINT DF_pos_stk_adj_sessions_created_at    DEFAULT GETDATE(),
        ...
        -- Constraints
        CONSTRAINT PK_pos_stk_adj_sessions        PRIMARY KEY (adj_id),
        CONSTRAINT UQ_pos_stk_adj_sessions_adj_no UNIQUE      (adj_no),
        CONSTRAINT CK_pos_stk_adj_sessions_status CHECK       (status IN ('Draft','Posted','Reversed')),
        CONSTRAINT CK_pos_stk_adj_sessions_type   CHECK       (adj_type IN (
            'Physical Count','Damage Write-Off','Found/Excess',
            'Price Update','Location Transfer','Opening Stock')),
        CONSTRAINT CK_pos_stk_adj_sessions_counters CHECK (
            total_lines    >= 0 AND qty_increases >= 0 AND qty_decreases >= 0
            AND price_changes >= 0 AND location_changes >= 0),

       
        CONSTRAINT FK_pos_stk_adj_sessions_created_by
            FOREIGN KEY (created_by)  REFERENCES pos_users (id),
        CONSTRAINT FK_pos_stk_adj_sessions_posted_by
            FOREIGN KEY (posted_by)   REFERENCES pos_users (id),
        CONSTRAINT FK_pos_stk_adj_sessions_reversed_by
            FOREIGN KEY (reversed_by) REFERENCES pos_users (id)
    );

    -- Indexes
    CREATE INDEX IX_pos_stk_adj_sessions_adj_date
        ON pos_stk_adj_sessions (adj_date DESC);

    CREATE INDEX IX_pos_stk_adj_sessions_status
        ON pos_stk_adj_sessions (status, adj_date DESC);

    CREATE INDEX IX_pos_stk_adj_sessions_warehouse
        ON pos_stk_adj_sessions (warehouse_id, adj_date DESC);

    CREATE INDEX IX_pos_stk_adj_sessions_created_by
        ON pos_stk_adj_sessions (created_by);

    PRINT 'Created table: pos_stk_adj_sessions';
END
ELSE
    PRINT 'Table already exists: pos_stk_adj_sessions';
GO
...
BEGIN
    CREATE TABLE stk_adj_lines (
        ...
        CONSTRAINT FK_stk_adj_lines_adj
            FOREIGN KEY (adj_id)     REFERENCES pos_stk_adj_sessions (adj_id),
...
BEGIN
    CREATE TABLE stk_adj_audit (
        ...
        CONSTRAINT FK_stk_adj_audit_changed_by
            FOREIGN KEY (changed_by) REFERENCES pos_users (id)
    );
...
-- ============================================================
-- 4. pos_product_locations — Multi-location stock per bin
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'pos_product_locations')
BEGIN
    CREATE TABLE pos_product_locations (
        ...
        qty_at_location     DECIMAL(18,3)   NOT NULL CONSTRAINT DF_pos_product_locs_qty DEFAULT 0,

        is_primary          BIT             NOT NULL CONSTRAINT DF_pos_product_locs_primary DEFAULT 0,
        last_counted_at     DATETIME        NULL,
        last_counted_by     INT             NULL,

        -- Constraints
        CONSTRAINT PK_pos_product_locations       PRIMARY KEY (loc_id),
        CONSTRAINT UQ_pos_product_locations_bin   UNIQUE      (product_id, warehouse_id, aisle_code, shelf_code, bin_code),
        CONSTRAINT CK_pos_product_locs_qty        CHECK       (qty_at_location >= 0),

        CONSTRAINT FK_pos_product_locs_product
            FOREIGN KEY (product_id)  REFERENCES pos_products (id),
       
        CONSTRAINT FK_pos_product_locs_counted_by
            FOREIGN KEY (last_counted_by) REFERENCES pos_users (id)
    );

    -- Indexes
    CREATE INDEX IX_pos_product_locs_product
        ON pos_product_locations (product_id);

    CREATE INDEX IX_pos_product_locs_warehouse
        ON pos_product_locations (warehouse_id);

    CREATE INDEX IX_pos_product_locs_location_code
        ON pos_product_locations (location_code);

    CREATE INDEX IX_pos_product_locs_primary
        ON pos_product_locations (product_id, is_primary) WHERE is_primary = 1;

    PRINT 'Created table: pos_product_locations';
END
ELSE
    PRINT 'Table already exists: pos_product_locations';
GO
        ...
        SELECT @NextSeq = ISNULL(MAX(
            TRY_CAST(
                SUBSTRING(adj_no,
                    ...
                    4)
            AS INT)), 0) + 1
        FROM pos_stk_adj_sessions WITH (UPDLOCK, HOLDLOCK)
...
SELECT
    ...
    ISNULL(uc.name, uc.username)   AS created_by,
    s.created_at,
    ISNULL(up.name, up.username)   AS posted_by,
    s.posted_at
FROM  pos_stk_adj_sessions s

JOIN  pos_users      uc ON uc.id = s.created_by
LEFT  JOIN pos_users up ON up.id = s.posted_by
LEFT  JOIN stk_adj_lines l ON l.adj_id = s.adj_id
GROUP BY
    s.adj_id, s.adj_no, s.adj_date, s.adj_type, s.status,
    s.warehouse_id, s.notes, s.created_at, s.posted_at,
    uc.name, uc.username, up.name, up.username;
GO
...
SELECT
    ...
    a.old_value,
    a.new_value,
    ISNULL(u.name, u.username)            AS changed_by,
    a.reason,
    s.adj_type,
    s.status                                   AS session_status
FROM  stk_adj_audit     a
JOIN  pos_products      p ON p.id  = a.product_id
JOIN  pos_users             u ON u.id  = a.changed_by
JOIN  pos_stk_adj_sessions  s ON s.adj_id = a.adj_id;
GO
