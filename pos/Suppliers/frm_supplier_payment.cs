...
namespace pos
{
    public partial class frm_supplier_payment : Form
    {
        ...
        public frm_supplier_payment(frm_addSupplier mainForm, int supplier_id)
       {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            if (_invoice_no != string.Empty && _supplier_id != 0)
            {
                ...
                int entry_id = Insert_Journal_entry(_invoice_no, payable_account_id, Convert.ToDouble(txt_total_amount.Text), 0, txt_payment_date.Value.Date, txt_description.Text, 0, 0, 0);
                ...
                Insert_Journal_entry(_invoice_no, cash_account_id, Convert.ToDouble(txt_total_amount.Text), 0, txt_payment_date.Value.Date, txt_description.Text, 0, _supplier_id, entry_id);
                
                if (txt_discount.Text != string.Empty)
                {
                    ///receivable JOURNAL ENTRY (debit)
                    Insert_Journal_entry(_invoice_no, payable_account_id, Convert.ToDouble(txt_discount.Text), 0, txt_payment_date.Value.Date, txt_description.Text, 0, 0, 0);
                    
                    /// SALES DISCOUNT JOURNAL ENTRY (credit)
                    int entry_idd = Insert_Journal_entry(_invoice_no, purchases_discount_acc_id, 0, Convert.ToDouble(txt_discount.Text), txt_payment_date.Value.Date, txt_description.Text, 0, 0, 0);
                    
                    
                    ///ADD ENTRY INTO supplier PAYMENT(debit)
                    Insert_Journal_entry(_invoice_no, purchases_discount_acc_id, Convert.ToDouble(txt_discount.Text), 0, txt_payment_date.Value.Date, txt_description.Text, 0, _supplier_id, entry_idd);
