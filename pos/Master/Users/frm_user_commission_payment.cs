...
namespace pos
{
    public partial class frm_user_commission_payment : Form
    {
        ...
        public frm_user_commission_payment(frm_adduser mainForm, int user_id)
       {
        ...
        {
            if (_invoice_no != string.Empty && _user_id != 0)
            {

                int entry_id = Insert_user_commission(_invoice_no, 0, Convert.ToDouble(txt_total_amount.Text), 0, txt_payment_date.Value.Date, txt_description.Text);
        ...
        private int Insert_user_commission(string invoice_no, int account_id, double debit, double credit, DateTime date,string description)
        {
            int journal_id = 0;
