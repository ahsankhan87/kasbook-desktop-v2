...
{
    /// <summary>
    /// Moving Items Report Form
    /// Displays inventory items with high sales velocity (recent transactions).
    /// Features: Filter by days, category, brand, location. Export to CSV, Print, and Summary statistics.
    /// </summary>
    public partial class FrmMovingItemsReport : Form
    {
        private DataTable _currentData;
        private MovingItemsReportBLL _bll;

        public FrmMovingItemsReport()
        {
            InitializeComponent();
            _bll = new MovingItemsReportBLL();
        }

        private void FrmMovingItemsReport_Load(object sender, EventArgs e)
        {
            try
            {
                // Apply theme
        ...
        private void LoadReport()
        {
            try
            {
                using (BusyScope.Show(this, "Loading Moving Items Report..."))
                {
                    ...
                    // Fetch data
                    _currentData = _bll.GetMovingItems(branchId, daysThreshold, 0, categoryCode, brandCode, locationCode);
        ...
        private void UpdateSummary()
        {
            try
            {
                if (_currentData == null || _currentData.Rows.Count == 0)
                {
                    ...
                    lblAvgTurnover.Text = "Avg Days to Turnover: 0";
                    return;
                }
                ...
                lblAvgTurnover.Text = $"Avg Days to Turnover: {avgDays:N0}";
            }
            catch (Exception ex)
            {
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SaveFileDialog sfd = new SaveFileDialog
                {
                    ...
                    DefaultExt = "csv",
                    FileName = $"MovingItems_{DateTime.Now:yyyyMMdd_HHmmss}.csv"
                })
                {
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        {
                            string result = _bll.ExportMovingItemsToCSV(_currentData, sfd.FileName);
            ...
            try
            {
                using (PrintDialog pDialog = new PrintDialog())
                {
                    if (pDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        {
                            PrintDataGridView(dgvReport, "Moving Items Report");
                        }
                    }
                }
            }
